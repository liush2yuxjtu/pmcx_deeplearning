# config_example.py - Example configuration for custom label mappings

# This is an example configuration file showing how to customize label mappings
# To use this, copy it to config.py and modify as needed
...
tissue_mappings = {
    ...
    # 可以添加自定义分割工具的映射
    'custom_tool': {
        'cortex': [10, 11],       # 自定义工具的皮层标签
        'scalp': [20],            # 自定义工具的头皮标签
        'skull': [30, 31],        # 自定义工具的颅骨标签
        'csf': [40],              # 自定义工具的脑脊液标签
        'wm': [10],                # 自定义工具的白质标签
        'gm': [11],                # 自定义工具的灰质标签
        'air_cavities': [60, 61],  # 自定义工具的空气腔室标签
    }
}
