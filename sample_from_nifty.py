...
class SampleConfig:
    ...
    def _load_valid_eeg_positions(self) -> List[str]:
        ...
        try:
            df = pd.read_csv(self.full_eeg_csv_path)
            # Assuming the first column contains the EEG position names
            positions = df.iloc[:, 0].tolist()
            # Filter out any NaN or empty values
            positions = [pos for pos in positions if isinstance(pos, str) and pos.strip()]
            return positions
