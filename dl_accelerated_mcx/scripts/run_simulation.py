"""
Main entry point for DL Accelerated MCX Simulation
"""
import os
import sys
import argparse
...
from src.utils.data_processing import load_nifti, save_nifti, create_dummy_volume
...
Examples:
  ...
  python run_simulation.py --mode simple --data-path /path/to/data.nii.gz
  
  # Run in full mode (high quality but slow)
  python run_simulation.py --mode full --data-path /path://path/to/data.nii.gz
  
  # Run in DL accelerated mode (fast with high quality)
  python run_simulation.py --mode dl_accelerated --data-path /path/to/data.nii.gz
        """
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to segmentation file (for target mode)"
    )
...
def run_mcx_simulation(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stage2_inputs = {
        ...
        'custom_src': None,  # Add custom source parameters if needed
        'save_path': save_path,
        'p': config['power'],
        't': config['time'],
        'path': config['data_path'],
...
def run_dl_accelerated_simulation(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Simple mode MCX simulation completed")
...
def main() -> None:
    ...
    print("\nTo view results:")
    print(f"  - NIfTI results: {os.path.join(config['output_dir'], 'results_' + config['mode'], 'MCX_results_log.nii.gz')}")
    print(f"  - Input volume: {os.path.join(config['output_dir'], 'results_' + config['mode'], 'MCX_input_vol.nii.gz')}")
