...
# -*- coding: utf-8 -*-
"""
Sample script for loading energy maps and generating samples
Uses VAE_Transform to pre-process energy maps and generate samples
"""
...
class SampleConfig:
    ...
    # Input settings - using energy maps directly
    energy_dir: str = "energy_maps"  # Directory containing energy maps
    energy_file_prefix: str = "m2m_IXI012-HH-_MCX_AF3_p250_t8_results_log"  # Energy map file prefix
    is_log_scaled: bool = False  # Whether energy maps are already log-scaled (log(10+1))
    modality: str = "ct"  # Modality type (mri/ct)
    
    # Output settings
    output_dir: str = "outputs_energy"
    ...
    if mock_data is not None:
        ...
        with torch.amp.autocast(device_type='cuda'):
            for sample_per_batch in tqdm(mock_data):
                ...
                mock_data_collect.append(sample_per_batch)
            mock_data = torch.cat(mock_data_collect, dim=0)
        # FIXED: mock_data is simple_latent, correctly assigned to "input"
        output_dict["input"] = mock_data
    if mock_noise is not None:
        mock_noise_collect = []
        with torch.amp.autocast(device_type='cuda'):
            for sample_per_batch in tqdm(mock_noise):
                ...
                mock_noise_collect.append(sample_per_batch)
            mock_noise = torch.cat(mock_noise_collect, dim=0)
        # FIXED: mock_noise is full_latent, correctly assigned to "target"
        output_dict["target"] = mock_noise
        return output_dict
...
def load_and_preprocess_energy(config):
    """Load and preprocess energy maps using precomputed latent files"""
    print("Loading and preprocessing energy maps...")
    
    # Use precomputed latent files from temp_sample/inputs directory
    latent_dir = "temp_sample/inputs"
    simple_latent_path = os.path.join(latent_dir, f"{config.energy_file_prefix}_simple.pt")
    full_latent_path = os.path.join(latent_dir, f"{config.energy_file_prefix}_full.pt")
    
    # Check if latent files exist
    if not os.path.exists(simple_latent_path) or not os.path.exists(full_latent_path):
        raise FileNotFoundError(f"Latent files not found at {latent_dir}")
    
    # Load latent files directly
    print(f"Loading latent files from: {latent_dir}")
    print(f"  - {simple_latent_path}")
    print(f"  - {full_latent_path}")
    
    # Load and preprocess latents
    simple_latent = torch.load(simple_latent_path, map_location=config.device, weights_only=False).float()
    full_latent = torch.load(full_latent_path, map_location=config.device, weights_only=False).float()
    
    print(f"Loaded latent shapes:")
    print(f"  - simple_latent: {simple_latent.shape}")
    print(f"  - full_latent: {full_latent.shape}")
    
    # Return the latents directly, skipping the encoding step
    # This is because we're already loading precomputed latents
    return {
        "image": full_latent,
        "source_image": simple_latent
    }

def encode_energy_to_latent(preprocessed_data, autoencoder, config):
    """Encode preprocessed energy maps to latent space using autoencoder"""
    print("Encoding energy maps to latent space...")
    
    # Since we're already loading precomputed latents, we just need to scale them
    one_real_latent = {}
    with torch.no_grad(), torch.amp.autocast(device_type='cuda'):
        for k, v in preprocessed_data.items():
            one_real_latent[k] = v * 0.25
    
    print("Encoded latent shapes:")
    for k, v in one_real_latent.items():
        print(f"  {k}: {v.shape}")
    
    return one_real_latent
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Sample from energy maps with optional debugging")
    ...
    trainer = MyTrainer(
        rectified_flow,
        dataset=[0] * 1,  # Placeholder dataset
        batch_size=1,
        num_samples=1,
    ...
    # Load and preprocess energy maps
    preprocessed_data = load_and_preprocess_energy(config)
    
    # Encode energy maps to latent space
    one_real_latent = encode_energy_to_latent(preprocessed_data, autoencoder, config)
