...
class IXI_MCX_paired(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        return {
            "image":load_func(image_dict["image"]),
            #"source_image":load_func(image_dict["source_image"]),
            "cond":load_func(image_dict["source_image"]),
        }
...
class monai_wrapper(torch.nn.Module):
    ...
    def forward(self,x,times,cond=None,context=None):
        #context = cond 
        ...
        out= self.model(model_input,context=context,timesteps=timesteps)
        return out 
...
rectified_flow = RectifiedFlow(
    ...
    noise_schedule=lambda x:torch.sigmoid(torch.randn_like(x)),
    immiscible=False)
...
class MyTrainer(Trainer):
    ...
    def _process_input(self,data):
        ...
        if isinstance(data,tuple) or isinstance(data,list):
            ...
            img=data["image"] if "image" in data.keys() else None
            noise=data["source_image"] if "source_image" in data.keys() else None
            cond = data["cond"] if "cond" in data.keys() else None
        return img,noise,cond
    ...
    def sample(self, fname):
        ...
        mock_cond = mock_cond.repeat(self.num_samples//mock_cond.shape[0],1,1,1,1) if hasattr(mock_cond,"shape") else None
...
trainer = MyTrainer(
    ...
    checkpoints_folder="./checkpoints_lowres_simple2full_v2",
    results_folder = './results_lowres_simple2full_v2',  # samples will be saved periodically to this folder
)
